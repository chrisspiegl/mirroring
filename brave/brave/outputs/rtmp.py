...
class RTMPOutput(Output):
    ...
    def create_elements(self):
        ...
        if config.enable_video():
            # key-int-max=60 puts a keyframe every 2 seconds (60 as 2*framerate at 30fps)
            # key-int-max=48 puts a keyframe every 2 seconds (48 as 2*framerate at 24fps)
            pipeline_string += ' ' + self._video_pipeline_start() + \
                'x264enc name=video_encoder key-int-max=48 pass=cbr speed-preset=ultrafast tune=zerolatency bitrate=4500 ! h264parse ! queue ! mux.'
    ...
    def create_caps_string(self):
        ...
        # framerate=24/1 because I prefer this frame rate and it's less to render.
        # profile=baseline may be superflous but some have recommended it for Facebook
        return super().create_caps_string(format='I420') + ',framerate=24/1,profile=baseline'
