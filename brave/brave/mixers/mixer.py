...
class Mixer(InputOutputOverlay):
    ...
    def create_elements(self):
        ...
        if config.enable_video():
            ...
            pipeline_string += ('videotestsrc is-live=true name=videotestsrc ! videoconvert ! videoscale method=bilinear ! '
            ...
            # TODO: Possibly give option to start mixer without videotestsrc so that it is possible to have this element transparent and mix multiple mixers

        if config.enable_audio():

            # const QUEUE = "queue max-size-time=100 max-size-bytes=100 max-size-buffers=100"
            pipeline_string += \
    ...
    def handle_updated_props(self):
        if hasattr(self, 'pattern'):
            ...
            self.mixer_element['video'].set_property('background', self.background)
